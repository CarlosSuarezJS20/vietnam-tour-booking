  ...
  return (
    <div className="flex flex-col group cursor-pointer">
      ...
      <p className="text-sm text-gray-400 font-sans">
        {tour.duration} · {priceValue ?? tour.price}
      </p>
    </div>
  );
}
            ...
            <div className="flex flex-col gap-2">
              {(Object.keys(CATEGORIES) as CategoryKey[]).map((key) => (
                <button
                  ...
                  className={`text-left text-base font-semibold px-4 py-2.5 border transition-colors font-sans ${
                    activeKey === key
          ...
          {isError ? (
            ...
            <>
              <div className="relative flex items-center gap-3">
                <button
                  onClick={scrollPrev}
                  className="flex-none w-9 h-9 flex items-center justify-center border border-gray-300 text-gray-500 hover:border-brand hover:text-brand transition-colors"
                >
