...
const convertToTextFormat = (itinerary) => {
    ...
    if (days.length === 0)
        return '';
    return days
        .map((d) => `Day ${d.day}${d.activity || d.title ? `: ${d.activity || d.title}` : ''}${d.description || d.body ? `\n${d.description || d.body}` : ''}`)
        .join('\n\n');
};
const main = async () => {
    try {
        ...
        for (const tour of tours) {
            if (!tour.itinerary || tour.itinerary.trim() === '') {
                // Delete tours with empty itinerary
        ...
        for (const cruise of cruises) {
            if (!cruise.itinerary || cruise.itinerary.trim() === '') {
                // Delete cruises with empty itinerary
        ...
        console.log(`Total: ${toursDeleted + cruisesDeleted} deleted, ${toursUpdated + cruisesUpdated} updated`);
    }
    catch (error) {
